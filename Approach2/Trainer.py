    ...
    def _build_model(self):
        if isinstance(self.model, list):
            ...
            if not isinstance(self.model[0], nn.Module) and not isinstance(self.model[1], nn.Module):
                raise TypeError(
                    "the 2 models must be of type toch.nn.Module, " f"but found {type(self.model).__name__}"
                )
    ...
    def _build_loss_fn(self):
        if isinstance(self.loss_fn, list):
            ...
            if not isinstance(self.loss_fn[0], nn.Module) and not isinstance(self.loss_fn[1], nn.Module):
                raise TypeError(
                    "the 2 loss_fns must be of type toch.nn.Module, " f"but found {type(self.loss_fn).__name__}"
                )
...
class Model(BuildModel):
    ...
    def validate(self, val_loader: Iterable, metrics: Dict[str, Metric]):
        pass
