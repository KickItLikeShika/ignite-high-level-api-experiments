...
def training(local_rank, config):
    ...
    model, metrics, checkpoint = create_model_and_handlers(config, run_validation, output_transform)

    train_dataset, _ = get_dataflow(config)
    
    model.set_train_data(train_dataset, batch_size=config["batch_size"], num_workers=config["num_workers"], 
                    pin_memory=True, shuffle=True, drop_last=True)

    model.set_validation_data(train_dataset, batch_size=config["batch_size"], num_workers=config["num_workers"], 
                    pin_memory=True, shuffle=True, drop_last=True)

    model.attach_train_on_event(checkpoint, Events.COMPLETED)

    for name, metric in metrics.items():
        model.attach_train(metric, name)
        model.attach_validation(metric, name)

    model.fit(num_epochs=config["num_epochs"], validation=True, validate_every=1)
...
def create_model_and_handlers(config, run_validation, output_transform):
    ...
    model = Model(model, optimizer, criterion, ddp=True)
