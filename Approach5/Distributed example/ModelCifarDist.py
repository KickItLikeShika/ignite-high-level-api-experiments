...
def run(
    ...
    **spawn_kwargs,
    ...
    model.fit(train_dataset, num_epochs=config["num_epochs"])
...
def get_dataflow(config):
    ...
    train_dataset = {'dataset': train_dataset, 'batch_size': config["batch_size"], 
                     'num_workers': config["num_workers"], 'shuffle': True, 'drop_last': True}

    test_dataset = {'dataset': test_dataset, 'batch_size': 2 * config["batch_size"], 
...
def create_model(config):
    ...
    model = Model(model, optimizer, criterion, device=idist.device(), ddp=True)
    return model
